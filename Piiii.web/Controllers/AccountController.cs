...
namespace Piiii.web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(int? id, domain.user user)
        {
            ViewBag.ID = id;

            var userdetail = db.user.Where(m => m.email == user.email && m.password == user.password).FirstOrDefault();
            if (userdetail == null)
            {
                return View();
            }
            else
            {
                ...
                else if (userdetail.userId == 2)
                {
                    Session["userId"] = userdetail.userId;
                    return RedirectToAction("Index", "Missionemp");
                }
                else 
                {
                    Session["userId"] = userdetail.userId;
                    return RedirectToAction("Index", "Missionrh");
                }
